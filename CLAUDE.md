...
Circle 0 design work (BE-000, BE-002 to BE-004) is complete; BE-010 (Python scaffold) and BE-011 (`make backend-*` targets; `make backend-verify` is the real backend gate) are done; BE-012 wrote the backend CI workflow but it has never run (CI green pending a maintainer push, so the Circle 1 gate is open); BE-020 (typed configuration, `.env.example`) and BE-021 (app factory; clock injection deferred to BE-034) are done; BE-022 (internal caller authentication) and BE-023 (request context, redacted logging; audit and worker propagation deferred) are done; BE-024 (problem details) and BE-025 (readiness semantics; real dependency probes pending) are done; Circle 2 is complete except two named gaps (see its gate note); `contracts/openapi.json` is generated by `make openapi-generate` and checked by `make openapi-check`. BE-030 (Compose infrastructure; ClamAV not yet exercised) is done; BE-031 (async database kernel; integration tests run via `make backend-integration` against `make infra-up-core`) is done; BE-032 (roles and grant baseline; proven on probe tables) is done; BE-033 (Alembic baseline, `make migrate`/`make db-roles`) is done; BE-034 (clock, UUIDv7, cursors, idempotency) is done, so Circle 3 is complete except the unexercised ClamAV service (see its gate note); BE-040 (locality, project, translation model) is done; BE-041 (sources, versions, citations, publication service) is done; BE-042 (escalation routes, trust metadata) is done; BE-043 is blocked on BE-001, so BE-044 (public project endpoints; the API now connects as `shaidago_public` via `DATABASE_URL_PUBLIC`) is done; BE-045 (contract and query quality) is done, so Circle 4 is complete except the seed pipeline: BE-043 is blocked until BE-001 lands. BE-050 (reviewer users, audit log, bootstrap; session revocation on disablement lands in BE-051) is done; BE-051 (sessions, CSRF derivation, cookie contract) is done; BE-052 (reviewer session and CSRF dependency, `DATABASE_URL_REVIEWER`) is done; BE-053 (capability policy) is done; BE-054 (sign-in/out, rate limits) is done, so Circle 5 is complete except the BFF's final `Set-Cookie` proof; BE-060 (envelope encryption, data keys, `make kek-rotate`) is done; the pure core of BE-062 (tracking codes) is done ahead of BE-061; the next task is BE-061 (private report persistence). BE-001, the six-project source register, is deferred by the maintainer to just before BE-043, and the Circle 0 exit gate stays open until it lands. No seed data may be presented as factual before that register exists. Never fabricate project data, sources, escalation routes, or translations.
