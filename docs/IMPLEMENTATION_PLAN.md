...
| AI                       | OpenAI Responses API with Structured Outputs; configurable model IDs      | Strict output schemas, multilingual responses, and provider isolation. Use`gpt-6-astra` for the few complex discovery syntheses, `gpt-5.6-terra` for short Q&A/explanations, and `text-embedding-3-small` for retrieval unless evals justify a change. |
...
Seed embeddings are checked in at `data/embeddings/<model>.jsonl`, keyed by chunk content hash, and regenerated by `make embeddings` only when a key is present. `make seed-demo` loads them without an OpenAI key. If a chunk has no stored embedding and no key is configured, retrieval falls back to PostgreSQL full-text search only, and the answer metadata records `retrieval_mode: "keyword"` so the degradation is visible and tested.
...
- OpenAI and Brave keys: worker/API environment secrets only.
...
Without OpenAI or Brave keys, explicit fixture adapters keep the full workflow runnable using checked-in, clearly labelled demo responses. The UI displays “demo replay” so recorded data is never misrepresented as a live search. At least one pre-submission run must exercise the real providers and preserve only safe evidence of success.
...
4. Railway, R2, Brave Search, and OpenAI credentials for the hosted demo.
