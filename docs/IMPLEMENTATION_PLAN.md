...
Seed embeddings are checked in at `data/embeddings/<model>.jsonl`, keyed by chunk content hash, and regenerated by `make embeddings` only when a key is present. `make seed-demo` loads them without any provider key (Groq serves no embeddings; see ADR-0009). If a chunk has no stored embedding and no key is configured, retrieval falls back to PostgreSQL full-text search only, and the answer metadata records `retrieval_mode: "keyword"` so the degradation is visible and tested.
