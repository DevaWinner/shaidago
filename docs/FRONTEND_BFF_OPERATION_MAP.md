...
| `reviewer_publication_withdraw` | Withdraw a draft update. | `POST app/api/reviewer/reports/[reportId]/public-updates/[updateId]/withdraw/route.ts`. | Empty body; the API defines none. | `BFF-REVIEWER-EMPTY` | Empty/safe acknowledgement only; no-store. |
