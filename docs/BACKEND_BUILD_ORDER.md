...
> **Execution status (2026-09-19): complete.** The Redis limiter is now an atomic sliding window (one Lua script using Redis's own clock, so every API process shares one clock; a refused attempt is not recorded, so hammering a limit never extends its lockout; keys expire with their window). `api/rate_policy.py` lists every limiter key with its subject, setting, window, and failure mode, and a test fails if a key appears in the source without a policy. New budgets: per-reviewer read and write budgets keyed by reviewer ID (`RATE_REVIEWER_READ_PER_MINUTE`, `RATE_REVIEWER_WRITE_PER_MINUTE`) applied to every reviewer route, and a shared Q&A provider budget (`RATE_QA_GLOBAL_PER_HOUR`) beside the per-client one; discovery already had a per-client, a per-reviewer, and a global daily budget. Redis unavailable is a `503` for every policy (fail closed): submission, tracking, sign-in, Q&A, discovery, and reviewer routes make no change and no provider call, the next request reconnects by itself, and the browser keeps its unsent draft. Trusted-proxy handling stays at the BFF: the API receives only the pseudonymous rotating client HMAC and never an IP address. 6 tests against real Redis (80 concurrent attempts across four connections admit exactly the limit, sliding and expiry behaviour, unreachable Redis) and 3 API tests pass.
...
> **Execution status (2026-09-19): complete.** `api/cache_policy.py` makes one cache decision for every response: only successful (or `304`) `GET` reads of `/v1/localities`, `/v1/projects`, `/v1/projects/{slug}`, and `/v1/projects/{slug}/sources/{id}` may carry a public cache header; every other response, including all errors, is forced to `no-store` (a route that says nothing gets `no-store`, and one that says `public` where it may not is corrected). Evidence: 14 unit tests of the middleware and the path allowlist; a source scan proving no module outside the public catalogue declares a public cache header, ETag, or `cacheable(` call; a test that requests every one of the 37 documented operations and asserts an explicit policy (`no-store` unless it is one of the four public reads); and a test that exercises the private success paths (tracking, report contact and detail, notes, evidence download, public-update drafts, discovery runs, health) and asserts `no-store`, with the public reads keeping `public`, validators, and `Vary`.
...
> **Execution status (2026-09-19): complete.** `tests/integration/test_schemathesis_reviewer.py` fuzzes all 20 reviewer operations against the generated contract with a real reviewer session and CSRF token (malformed and boundary bodies, Unicode, unknown fields, wrong content types, invalid commands, real database, reviewer role): it found one contract gap (the evidence download did not document its `422`), now fixed, and passes cleanly on repeated runs alongside the existing public-API fuzz. `tests/integration/test_public_projection_proof.py` builds a report with every private ingredient (description, contact, reporter handle, attachment, follow-up question and answer, reviewer note, private status reason, a report-scoped discovery run and source) and proves none of those canary values, the tracking code, the handle, or the report ID appears in any response body or header, any error body (including requests that try to echo them), or any log line, across the public catalogue, tracking, Q&A, discovery, handle, health, and unknown routes; the only exception is the reviewer's follow-up question, which is intended for the reporter and appears only in their own tracking response. An explicit field allowlist pins the public discovery DTOs, and the existing snapshot pins the public project shapes.
