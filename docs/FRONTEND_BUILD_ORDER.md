...
> **Execution status (2026-09-20): complete.** `/{locale}` is now a request-time Server Component (`connection()`, so the API is never called during a build and the tagged data cache stays on). It shows the purpose and place in the reviewed hero copy, a GET search to the directory, browse and private-report actions, the newest real public record (with its recorded status and a real last-checked date, or an honest sentence when the service is down or there are none), locality entry points from the API's area councils, category entry points only for categories that have records, a three-point trust promise linking to the trust page, three-step "how it works", and the prototype and not-an-emergency-service limitation. Nothing is invented: a test rejects count claims, partners, testimonials, and awards, and no social preview image is set because none is approved. Works with no JavaScript. The fictional example record and its evidence rail were removed. Limitations: the record link and the trust and report links go to routes later circles build and show the safe not-found page until then; the hero's `landing` copy is the maintainer's reviewed translation, while the new `home` domain is untranslated (`null`) until the end-of-build pass.
...
> **Execution status (2026-09-20): complete.** `src/lib/directory/filters.ts` parses and writes `q`, `locality`, `category`, `status`, `verification`, and `cursor` with strict allowlists that are type-checked against the generated API enums: unknown values, malformed or oversize cursors, repeated parameters, and stray names are dropped, so nothing typed becomes a backend filter, and a non-canonical URL is redirected to its canonical form (adding `#results`, which a plain GET submission drops). The form is a plain GET form; active filters are removable links and "Clear all filters" is one link; results take keyboard focus after a change through a 10-line client component, and the page works without it. 11 unit tests cover hostile values, repeated parameters, round-trip, canonical order, and the bounded API query.
...
> **Execution status (2026-09-20): complete.** `/{locale}/projects` fetches on the server and renders 12 records per page as cards with one named action (the title), plain facts in a definition list (locality, category, recorded status, last checked), and a stale note after 90 days; states cover results, first-load empty, no matches, invalid cursor recovery, and an unavailable service (with retry) without ever showing backend text. Pagination follows the opaque cursor with next and first-page links, filters survive it, and a language switch keeps filters but drops the cursor. The API's summary carries no source count or verification value, so cards show neither rather than invent them. A fallback or machine-assisted text notice appears when the API served the original language. Finding: `loading.tsx` had to be removed because Next streams its placeholder and swaps content in with inline script, which left no-JavaScript visitors on "Preparing this page". 15 browser tests per browser and 8 component tests cover these states, including with JavaScript disabled.
...
> **Execution status (2026-09-20): complete.** Public reads go through `unstable_cache` wrappers in `src/lib/api/public-data.ts`, keyed by language and filters, fresh for 60 seconds, and tagged `public-catalogue`; only successful reads are stored (failures throw and are never cached). Finding: Next refuses to cache a fetch carrying an `Authorization` header once a route is dynamic, whatever `revalidate` says, and `force-dynamic` also forces no-store, so caching at the fetch level silently did nothing; the wrapper fixes it and both pitfalls are documented in code. A successful reviewer publication expires the tag immediately (`revalidateTag(tag, { expire: 0 })`), and no other command does. Tests: every method of the server API is classified as a public read (tagged, cached) or never cached (`cache: "no-store"`), so a new method must be placed deliberately; the browser probe shows identical reads reach the API once, each language and each filter set is its own entry, an outage is not cached, and public pages call only the two public endpoints. Not built: a visible page-level "revalidating" indicator (the cache is invisible to the page); freshness is shown per record by its last-checked date.
...
> **Execution status (2026-09-20): complete.** Axe passes for the directory in all four languages across five states (results, filtered, no matches, unavailable, invalid cursor); each language reflows at 320 px and at 200% text with no horizontal scroll; interactive targets are at least 44 px; results are a labelled region with one short polite count and card links each named by their own title; reduced motion leaves no animation. First-load JavaScript, measured in a real browser as gzip of every script loaded, is 142.9 KB for the landing and 152.2 KB for the directory, against the plan's 170 KB budget, and a test enforces it. The first measurement was 172 to 173 KB: a 31 KB chunk with the whole dialog implementation loaded on every page for the rare unsaved-draft warning, which is now loaded only when needed. Filtering is a GET form and links; the directory's only client code is the results-focus helper and the shared field components. Not proven: real screen-reader speech, and true browser zoom (emulated by text size and a 320 px viewport).
...
> **Gate status (2026-09-20): closed with caveats.** Met: the landing states purpose, place, trust promise, and next action from real data only; the directory works with JavaScript disabled and its URL state is shareable and canonical; empty, no-match, invalid-cursor, unavailable, stale, and pagination states exist and are tested; public caching is language- and filter-keyed, stores only successes, is dropped on publication, and cannot include private operations (enforced by a classification test over the whole server API); mobile layout and the JavaScript budget are met and enforced. Caveats, not claimed: the new `home` and `directory` copy (116 keys per language across all pending domains) is `null` in Hausa, Igbo, and Yorùbá until the maintainer's end-of-build pass, so those pages currently show the English original with `lang="en"` under a fallback notice, and the requirement that these states exist in all locales is structural until then; the record, trust, and report links resolve to the safe not-found page until Circles 7 to 9; there is no page-level "revalidating" or offline-cached indicator yet (offline is a later circle); the browser suites use a fictional mock of the two public endpoints, while the real API was exercised by hand against its seeded records (12 cards, category filter, invalid-cursor recovery, and `/ha`); and no screen-reader run.
...
> **Execution status (2026-09-20): complete.** Opt-in only, after the shared-device warning. A draft holds the concern, the description, the step, and the project (the type has no place for anything else); it expires after 24 hours, is deleted when corrupt, expired, future-dated, or of another version, is offered for the same project only, has a visible delete action, and is cleared on success. Storage failures (missing, blocked, full) are caught, and the person is told nothing was saved. Without consent nothing is written. 12 unit tests, component tests inside the 13 wizard tests, and 2 browser tests cover this. A bug found on the way: a draft saved after the page opened was judged dated in the future against the page's opening time and deleted at once; a person who has consented is no longer re-judged.
...
> **Execution status (2026-09-20): complete.** `/{locale}/reviewer/sign-in` (dynamic, no-store, noindex) has labelled identifier and password fields with `username`/`current-password` autocomplete, a client-side required check, one generic message for every rejected credential, a visible back-off countdown after a rate limit (button held until it ends), and an unreachable-service message. Credentials go only to `POST /api/reviewer/session`; the password is emptied after every attempt and nothing is stored. The return target is rebuilt from an exact allowlist (`safeReviewerTarget`: this locale's queue or one report by UUID; no query, fragment, host, or other locale), and `?reason=` accepts only `expired` and `signed_out`. `reviewerOptionsFor`/`requireSession` (`src/lib/reviewer/session.ts`) read the HttpOnly cookie on the server and turn a `401` into a redirect to sign-in with no private data carried; a Server Component cannot clear the stale cookie, so the next sign-in overwrites it. Proof: 20 unit tests for the redirect allowlist, 5 component tests, and 6 browser flows on Chromium and mobile WebKit (no-store and noindex, generic failure with no cookie or storage, rate-limit hold, HttpOnly `Lax` cookies unreadable by the page, open-redirect refusal, reason handling), plus `make web-verify`. `ha`/`ig`/`yo` reviewer copy is `null` (pending) so those locales show the English original with a notice. Not proven: the real API.
...
> **Execution status (2026-09-20): complete.** `/{locale}/reviewer/reports` (dynamic, no-store, noindex, no prefetch: plain anchors) is server-rendered behind the session cookie: no cookie redirects to sign-in and an API `401` redirects to `sign-in?reason=expired` with nothing carried over. Filters (`status`, `risk`, `category`, `project`) and the opaque `cursor` live in the address, are checked against the API's own enumerations and pattern (unknown values are dropped, never forwarded), and a filter change starts at page one. Each row is a card, not a table: record name, concern, `Status:` and `Risk:` in words with a shape, received and status-changed dates in Lagos time, file count, open-question count, and whether contact details exist (never the value). No description, contact, note, or evidence appears. States: results (20 per page), empty, filter-empty with a clear link, stale cursor (`invalid_cursor`) with a way back, forbidden, rate-limited, and unavailable with a retry link; the filter form stays usable in each. Proof: 14 unit tests (filter parsing, query, address), 6 component tests (words not colour, contact/description never rendered, 50-row page, GET form), and 10 browser flows on Chromium and mobile WebKit (sign-in redirect, ended session, headers, cursor paging, filters, no-match, stale cursor, three failure outcomes, 320 px no sideways scroll, no prefetch of report pages), plus `make web-verify`. Not proven: the real API, a large real queue, and screen-reader output.
...
> **Execution status (2026-09-20): complete.** `/{locale}/reviewer/reports/{reportId}` (dynamic, no-store, noindex) renders in the specified order: status and risk, the fictional-data warning, the observation (the reporter's description as escaped plain text, plus the project record and a link to its public page), evidence metadata (cleaning and scan state in words, a `not_scanned_demo` warning, nothing rendered inline), contact behind an explicit reveal, status history with the reporter-visible message kept apart from the private internal reason, the reporter questions and answers, handle context stated as a pseudonym and not proof, and a Source Scout section that says plainly it is not available yet. Contact is a plain GET form (`reveal=contact`), so it needs no script and the value is never in the address; the API audits the reveal. When the account may not see contact, the report still loads and says why. A malformed identifier never reaches the API and looks the same as an unknown one; 401 redirects to sign-in with this report as the safe return target. States: loaded, not found, forbidden, rate-limited, unavailable with retry, signed out, ended session. Proof: 10 component tests (script and markup stay text, contact never in the DOM until revealed, evidence states, history/questions/handle) and 11 browser flows on Chromium and mobile WebKit (redirect and return target, ended session, headers, section order, plain-text description, reveal and hide with the audit entry seen, denied contact, evidence, history, identical not-found answers, 320 px, queue to detail), plus `make web-verify`. Notes, evidence download, status actions, and the public-update composer are added by FE-113 to FE-115. Not proven: the real API and screen-reader output.
...
> **Execution status (2026-09-20): complete.** Evidence is downloaded only when the reviewer presses a named button (`Download {file}`): the page makes no evidence request on load, each press makes a fresh authorised request through `GET /api/reviewer/reports/{id}/evidence/{id}`, the bytes are held in memory only long enough to hand to the browser's save dialog and then released, and nothing (no address, no signed link) is stored; nothing is rendered inline. A `401` says the session ended and offers sign-in; other failures use the reviewed message for their code. Notes are plain escaped text, oldest first with a "later notes" link, and a note whose key is gone says so. The note form is private and append-only in the UI: it needs text, refuses over 4,000 characters before sending, opens a confirmation stating the note cannot be edited or deleted, then posts JSON, clears the box, and refreshes the list; after a failure the text stays, and a dropped connection warns that the note may already have been saved. The follow-up question controls (ask, withdraw) follow the same confirmation and failure rules. A cross-origin POST is refused by the boundary. Proof: 12 component tests (confirmation before any request, request shape, no storage, failure and session-ended messages, markup, length, fresh download per press, escaped notes), 7 browser flows on Chromium and mobile WebKit (paged notes, add a note, markup and outage, cross-origin refusal, download on request and per press, expired download, ask and withdraw), plus `make web-verify`. Not proven: the real API's note length rule (API.md says 4,000, OpenAPI 4,500; the UI uses 4,000), large files, and screen-reader output.
...
> **Execution status (2026-09-20): complete.** The report detail's first section now holds the status controls (`StatusActions`, one client island). It offers only the commands `REVIEWER_TRANSITIONS` allows from the current status; a unit test compares that table with the `report_status` machine in `contracts/controlled-vocabulary.json` (reviewer rows only, so the reporter-only `record_follow_up` cannot be offered) and fails on drift. The form sends `expected_status` and `expected_version` exactly as rendered, so a stale view is refused rather than overwritten. Asking for information requires a reporter message and reopening requires a private reason (checked before anything is sent); the message is labelled as reporter-visible and never public. Every change opens a confirmation that says the reporter will see it and that nothing is published on the public record, with an extra warning for the consequential commands (close, refer, verify, reopen, request information). While a request is pending the fieldset and button lock so a second press cannot send twice. A `409` shows a calm explanation, keeps what was typed, and reloads only when asked; success is announced ("Status changed from X to Y. Nothing was published.") and the page data is refreshed so the new status, version, and history appear. Status change never publishes text: the response is `published: false` and the browser test proves no public-update or public-record request is made. Proof: 4 unit tests (contract parity), 8 component tests, and 7 browser flows on Chromium and mobile WebKit (allowed commands only, confirm then apply then history, reporter message versus private reason, reopen reason, stale-version conflict and recovery, `published:false` and no public request, keyboard confirm and Escape), plus `make web-verify`. Not proven: the real API's `409` payloads and a screen-reader run.
...
> **Execution status (2026-09-20): complete.** The last section of the report detail is a visually separate public-update composer (`PublicUpdatePanel`, one client island). It starts empty (private report text is never prefilled, and the guidance says not to copy it) and only offers citations that the public record for the report's own project already shows (`citationOptions`), sent with their exact passage and location so a passage cannot be typed or edited; with no approved citations, an unverified report, or an unloadable project it says why and offers no form. "Create a preview" posts the draft and renders the returned update with the same `PublicUpdateEntry` component that the public project timeline now uses (extracted from the record page into `src/components/project/update-entry.tsx` and reused there), plus its cited passages, the report status and version it was made against, and any blocking issues in words (guarded terms, private-report wording). Nothing is public until "Publish this update" is confirmed in a dialog that says what will appear; publish sends only the preview's digest, so what is confirmed is what was previewed. A stale digest (`409`) returns to editing with an explanation and publishes nothing; a publish that cannot proceed is unavailable and a forced click sends nothing. Earlier drafts are listed and reopened or discarded only on request, and success links to the public record. The response is parsed by hand (`parsePreview`) and refused if malformed. Also fixed here: the status controls now stay locked until a reload finishes, so a retry is made against the fresh version. Proof: 15 unit tests (preview parser, citation options, issue kinds), 10 component tests (empty composer, blocked states, validation, verbatim request, shared entry, issues block, confirm then digest-only publish, stale, discard, reopen, malformed and session-ended), and 6 browser flows on Chromium and mobile WebKit (approved citations only, exact preview and no publish request before confirmation, the published entry then appears on the public page with the same origin and verification label, flagged text blocked, a report changed after the preview makes the publish stale, reopen and discard a draft, 320 px), plus `make web-verify`. Not proven: a re-authentication step (the contract has none), the real API's issue codes beyond those in `docs/API.md`, and screen-reader output.
...
> **Execution status (2026-09-20): complete.** Proof across the reviewer workspace: 53 reviewer unit tests (redirect allowlist, queue filters, transition table with contract parity, publication parser), 66 reviewer component tests (including every allowed status transition from every status with the exact request body), 110 Chromium and mobile WebKit browser test runs (55 tests in 7 files: sign-in, queue, detail, notes and downloads, transitions, publication, and security; 6 of them skipped on iOS WebKit for Tab-key focus), and 30 axe runs (15 tests, each also checking 320 px reflow and 200% text for the state) across the sign-in, queue, and detail states in all four languages and the action confirmations, composer, preview, and publish confirmation in English. The full suites are `make web-e2e` 320 passed and 6 skipped and `make web-a11y` 312 passed. Security cases proven: no cookie, an ended session, and a session with no CSRF cookie are refused without reaching the API; a cross-origin POST is refused for the notes, status, question, and publish handlers; signing out clears the HttpOnly cookies, records the CSRF-checked API call, and going back or opening a report URL directly shows sign-in and no report; every reviewer page and evidence response is no-store; the console and storage hold no report, contact, note, or password text during a session; `/robots.txt` disallows the reviewer paths and the public home page links to none. Keyboard: a Chromium user signs in, opens a report from the queue, and reaches the actions, focus returns to the control that opened a confirmation, and the skip link moves focus to the main content. The axe run found and fixed a real defect: the report-detail failure and not-found states had no page heading. Base UI's nameless focus-guard spans on iOS WebKit are excluded from the dialog axe checks only (library internals; recorded for review). Not proven: the real API, a screen-reader pass, and iOS WebKit Tab-key focus (skipped, as elsewhere).
...
> **Gate status (2026-09-20): closed with caveats.** Met: the queue shows triage facts only and holds at 320 px; the private detail, contact (behind an explicit, audited reveal), notes, and evidence (a fresh authorised request per download) load only behind the session and the API's own policy, and an ended session leaves no private data; status transitions send the exact status and version they were shown and recover from a stale view without losing text; a public update is a separate, neutral-guidance composer using only approved citations, previewed with the public timeline's own component, and published only by confirming that exact preview's digest (a stale one is refused and nothing publishes); every reviewer page and response is no-store, unindexed, disallowed in `robots.txt`, unlinked from public pages, and absent from the client bundle scan. Evidence: `make web-verify`, `make web-e2e` 320 passed, `make web-a11y` 312 passed. Open: the real API was not run against these flows; reviewer copy in Hausa, Igbo, and Yoruba is pending (English shown with a notice); no screen-reader pass; the API has no re-authentication step for publishing, so none is offered (maintainer decision); a contact reveal is a GET whose history entry re-audits if revisited; iOS WebKit Tab-key focus is skipped; the per-report Source Scout section is a stated placeholder until Circle 12.
...
> **Execution status (2026-09-20): complete.** The project-detail Source Scout island posts only the public project slug to the purpose-built BFF, renders the no-private-input boundary before starting, parses only safe run fields, reports real stage/counts and replay/live state without a percentage, and keeps IDs/snapshots in component memory. It uses the shared polling model, stops at terminal states, and exposes a truthful “stop checking this page” control rather than claiming to cancel server work. Fallback locale keys stay structurally complete and visibly serve the English original until review. Three component tests, two new Chromium/mobile-WebKit E2E paths, and targeted axe/reflow coverage pass.
...
> **Execution status (2026-09-20): complete.** Results and analysis render from strict readers (`src/lib/discovery/parse.ts`; the OpenAPI types call `analysis` and `result` free-form, so the shape in `docs/IMPLEMENTATION_PLAN.md` and `data/discovery-fixtures/analysis.json` is checked field by field, and anything unmatched shows no analysis rather than part of one). At most ten cards (an eleventh makes the run unreadable and is refused), each labelled `discovered — not yet reviewed` by our own copy and never by server text, with publisher, preliminary type, publication date with its provenance and a date-conflict note, first-found and last-checked dates, availability, the permitted excerpt as inert text, a `noopener nofollow` original link (an address that is not plain http(s) gets no link), an injection warning when the API flags one, and, for reviewers, repeats grouped under the page they repeat (the public card has no duplicate field). Analysis stays in separate sections: supported facts with citations that resolve only to cards of this run, claims attributed to their publisher, both sides of any contradiction, gaps, and the safety note, with no rank, score, or confidence. Proof: 11 parser tests, 13 component tests, and browser flows for labels, grouping, the flagged page, the unsafe link, citation links, an unverifiable analysis, and the ten-source cap on the reviewer run and the public panel.
...
> **Execution status (2026-09-20): complete.** The reviewer run view shows up to five follow-up questions with why each is asked and its sensitivity. An answer, a skip, or an unsafe flag is one POST body to `/api/reviewer/discovery/{run}/follow-up-answers` with the question's index; the typed text lives only in component memory, is cleared once sent, and is never in the address, storage, console, or page afterwards (only a safe acknowledgement shows). Each question is bound to the run it was rendered for and refuses to send for another. An answer needs text unless skipped and is capped at 2,000 characters. Reporters answer their own reviewer-authored questions on the status page (FE-100); this panel is the reviewer's. Proof: component tests for privacy, the cap, skip and flag, and validation, and a browser flow whose private canary appears only in the one POST body, on Chromium and mobile WebKit.
...
> **Execution status (2026-09-20): complete.** Source decisions offer only what the contract's disposition machine allows (`not_reviewed`: attach, reject, defer; `deferred`: attach, reject; `attached` or `rejected`: reconsider). Each needs a reason of at most 400 characters and a confirmation stating that attaching creates a pending source and approves nothing, and a `409` reloads the run so the reviewer sees the current disposition. The card keeps its unreviewed label and stays listed after any decision, so discovery history stays visible, and an attached source is labelled "not verified". The run itself can be approved or rejected after confirmation. Proof: component tests (reason, confirmation, allowed decisions, conflict reload, run review) and browser flows for attach and run approval on Chromium and mobile WebKit. Not proven against the real API: the disposition names beyond `docs/API.md` and the controlled vocabulary.
...
> **Execution status (2026-09-20): complete.** Covered: no results, provider unavailable, fetch blocked or unsafe text, extraction failed, analysis invalid (no analysis shown), budget exhausted (the latest finished public run is shown with the reason), rate limit, cancelled with partial results, dead letter, a source flagged as instruction text, a run older than 24 hours, offline with the current results kept (checking resumes when back online), an outage with an explicit retry, and a run that needs a decision. Failure codes map to reviewed copy and unknown codes get a generic message; only a stage and the API's real counts are shown, never a percentage. Polling follows the 2/3/5/8/10-second schedule, pauses when the tab is hidden or the browser is offline, stops at a terminal status, and the status region changes only when the status does, so a screen reader is not flooded. Reviewers can cancel (confirmed) and stop or resume checking. Public and report-scoped runs cannot cross: each route returns `404` for the other's run ID, a run is fixed to its ID and scope in its component, and nothing is stored. Proof: 24 reviewer-run and 12 public-panel browser runs on Chromium and mobile WebKit (stages and stopped polling, cancel, failure, retry, offline, cap, isolation, nothing left after reload) and 18 axe runs across four languages and the failure, cancelled, results, and dialog states with 320 px and 200% text checks. Full suites: `make web-verify`, `make web-e2e` 358 passed and 6 skipped, `make web-a11y` 330 passed.
...
> **Gate status (2026-09-20): closed with caveats.** Met: a report-scoped run starts only from the exact query the reviewer approved (a changed query is refused and needs a new preview), and a private canary is absent from every browser request body, URL, storage, and console; every result stays labelled `discovered — not yet reviewed` until a reviewer decides, and even an attached source is shown as pending and not verified; analysis sections are separate and cite only cards of the same run, or are not shown at all; cancellation, failure, offline, and budget states are usable, and polling stops at a terminal status; and public and reviewer runs cannot be read through each other's routes or share state. Open: the real API and provider were not run (a fictional mock and recorded replay data only); the analysis shape is the documented one in the plan and fixtures, not OpenAPI-typed, so a backend change would show as "no analysis" rather than break the page (the backend should type it); reviewer and public Source Scout copy in Hausa, Igbo, and Yoruba is pending; no screen-reader pass; failure codes beyond those exercised in the fixtures fall back to a generic message.
...
> **Execution status (2026-09-20): complete.** `apps/web/app/manifest.ts` serves `/manifest.webmanifest`, linked from every page, with the real name and short name, the approved canvas colour (`#F7F2E8`) as theme and background, `/` as the start address (the locale proxy then picks the visitor's language without storing an identifier) and scope, standalone display, and no screenshot or shortcut. On the maintainer's instruction ("use the letter and make a simple icon and logo") the marks are a plain letter "S" with a ledger double rule, drawn from the approved accent and canvas colours with no font or external asset: `public/icons/icon.svg`, `icon-maskable.svg`, `logo.svg` (mark plus wordmark), and the committed PNGs `icon-192.png`, `icon-512.png`, `icon-maskable-512.png`, and `apple-touch-icon.png`, rendered by `scripts/render-icons.mjs`. The maskable icon keeps the glyph inside the safe zone. Three unit tests and two browser flows (Chromium and mobile WebKit) prove the manifest, that every declared icon is served with its type, and that the PNGs are real. Not proven: an actual browser install prompt, and this letter mark has had no maintainer visual review beyond the instruction to make it.
...
> **Execution status (2026-09-20): complete.** `apps/web/public/sw.js` uses native Service Worker and Cache Storage APIs only, with its cache policy in `public/sw-policy.js` (loaded by `importScripts`, and unit-tested by running the same file). It handles only same-origin GET navigations to the public catalogue (`/{locale}`, `/projects`, `/projects/{slug}`, `/projects/{slug}/sources/{id}`, `/trust`, `/offline`; filters `locality`, `category`, `status`, `verification`, `cursor` only, never `q`) and immutable `/_next/static` files; everything else, including every `/api` call, mutation, report, confirmation, tracking, handle, reviewer, authentication, Q&A, and discovery route, framework data request, and other origin, gets no `respondWith` and goes straight to the network. A response is stored only if it is a plain 200, not redirected or opaque, has no `Set-Cookie`, `Authorization`, `no-store`, or `private`, and is HTML for a page. Precache is the four offline pages and the files they name; caches are versioned (`sg-*-v1`), capped (40 pages, 150 files, 7 days), and older `sg-` versions are deleted on activate without touching anything else. The catalogue pages now send `Cache-Control: public, max-age=0, must-revalidate` (revalidated every time; the worker itself is `no-cache`) and private routes keep Next's no-store. HTML uses bounded stale-while-revalidate: a copy under five minutes old is served at once and refreshed in the background (not in low-data mode), an older one is refetched first with a five-second timeout and the copy is the fallback. Proof: 39 unit tests for the policy and 8 browser tests (Chromium and mobile WebKit): after visiting public, private, reviewer, and interactive routes and sending tracking and Q&A POSTs, every cache entry is allowlisted, GET-only, without private headers, and without any canary value (tracking code, contact, description, session, question). Not proven: production CDN behaviour.
...
> **Execution status (2026-09-20): complete.** A revisited public page shows a polite banner while offline ("a saved copy from {date} and may be out of date"), read from the time the worker stamped on the copy; the page itself still shows its own last-checked dates. An uncached public page shows the static, per-language offline page (precached with its own scripts so it runs offline the first time) with a retry, the list of saved pages read from this device's cache, a control to forget them, and a plain statement that reports, tracking, and reviewer actions need a connection and that nothing typed is sent, saved, or queued offline; private routes are not served from any cache, so they fail offline rather than pretend. A waiting service-worker update is announced in a polite status region and is applied only when the reader presses "Reload to use it"; on a report page it only says it will apply later, so an in-progress report is never lost, and the worker never calls `skipWaiting` by itself. Online and offline events are treated as hints for wording only. Old cache versions are removed on activate. Proof: a Chromium browser test of the offline revisit with the saved time, the offline page for an unvisited record, private routes unavailable, old-version cleanup that leaves foreign caches alone, forgetting saved pages, and worker headers; 8 axe runs at 320 px and 200% text for the offline page in four languages (`make web-a11y`). Limits: Playwright's WebKit cannot navigate offline with a service worker active, so offline behaviour is proven on Chromium only (the cache inspection runs on both); the update prompt is implemented but not driven end to end; the Hausa, Igbo, and Yoruba offline copy is pending review.
...
> **Execution status (2026-09-20): complete.** Every public page has a low-data switch backed by one functional cookie, `sg_low_data` (`1` or `0`; no identifier or personal data, one year, site-wide, `SameSite=Lax`, never sent to any API), documented in `src/lib/low-data.ts`. A four-line head script applies the stored choice to `<html data-low-data>` before first paint. In the mode: all transitions and animations are off, any element marked decorative or as a thumbnail is hidden (there is none today: the app uses system fonts and no images), the worker stops refreshing saved pages in the background, and Source Scout polling waits twice as long instead of stopping. All facts and actions stay: a directory search, the report form, and a public Source Scout run were exercised with the mode on. The browser's data-saving signal only shows a suggestion ("Turn on low-data mode?") that can be declined for the session; it never turns the mode on, and an explicit choice, on or off, is never overridden. Proof: 3 unit tests (cookie parsing and writing, the head script), 4 component tests, and 6 browser flows on Chromium and mobile WebKit (cookie attributes, attribute present before hydration, motion off and no overflow at 320 and 1280 px in four languages, task completion, no prefetch or framework-data requests, suggestion versus explicit off, slower polling of at least 3.5 seconds between checks). A real hydration mismatch was found and fixed on the way (a client-only value read during render reset the `<html>` attribute). Limits: the mode saves little today because the app already ships no images or web fonts; the Hausa, Igbo, and Yoruba copy is pending review.
...
> **Execution status (2026-09-20): complete.** Two real defects were found and fixed. Both Source Scout panels stopped polling after the first "not modified" answer (and the public one after the first skipped attempt or reconnect), because their effects only re-ran when the run changed; each attempt now schedules the next, and coming back online restarts the schedule at once. Browser reads of runs had no time limit; `src/lib/net/timeout.ts` now bounds them at ten seconds (with a fallback where `AbortSignal.any` is missing), after which the reader sees the failure and a retry. Already in place and re-proven: server-side timeouts per operation, idempotency keys reused on retried report submission and follow-up answers, upload progress and cancel, and starting a run being locked while it is in flight. Online and offline events only choose wording and pause or resume checking; they are never taken as truth. Proof: 4 unit tests for the timeout helper and 6 browser tests (Chromium and mobile WebKit unless noted): a flapping connection through three offline/online cycles resumes the same run and sends exactly one start request; a double press makes one request and one run; a run that keeps answering "not modified" keeps being checked at spaced intervals; offline before a question sends nothing, keeps the typed words, and succeeds on retry after reconnecting; and, on Chromium, a read that never answers is abandoned after the bounded wait and can be retried, and going back online after a saved page clears the offline notice. Existing suites cover the report path (offline before and after sending, retry with the same key, dropped connection, cancel). Not proven: a throttled "slow 3G" profile (the slow-answer case above is the stand-in), and the client timeout on WebKit, whose test runner does not release a route-held request when the page aborts it.
...
> **Gate status (2026-09-20): closed with caveats.** Met: the app has a manifest with the real name, the approved colours, and, on the maintainer's instruction, a plain letter-mark icon set (any, maskable, Apple touch) and logo; a recently viewed public project can be revisited offline with the time it was saved (Chromium proof); automated cache inspection after visiting public, private, reviewer, and interactive routes and sending tracking and Q&A requests finds only allowlisted public pages and framework files, with no private URL, body, header, cookie, or canary value; low-data mode removes motion and slows background refreshing and polling while every core action still works in four languages; and retried submissions and discovery starts do not duplicate work (one request and one run under a double press and under a flapping connection). Evidence: `make web-verify`, `make web-e2e` 395 passed and 9 skipped, `make web-a11y` 338 passed. Open: no real browser install prompt or mask preview was exercised, and the letter mark has had no visual review beyond the maintainer's request; Playwright's WebKit cannot navigate offline with a service worker active or release a route-held request on abort, so those two proofs are Chromium-only (skips are stated in the tests); the service-worker update prompt is implemented but not driven end to end; production CDN behaviour and a throttled network profile were not tested; Hausa, Igbo, and Yoruba copy for the offline page and low-data mode is pending review.
...
> **Update (2026-09-20): reviewed, disposition `fix`.** A fresh reviewer context that had not built the interface was given `docs/FRONTEND_FINISH_REVIEW_PACKET.md` and captured the hosted staging site itself. Its disposition is **`fix`**, covering only these named fixes: (1) English strings left on the Hausa landing; (2) a product-specific landing first viewport with a lead record and inline search; (3) a one-row mobile header with a distinct "Report a concern" control; (4) an explicit verification-state label and clearer "Not known" wording on the record summary, with the evidence rail directly after it on mobile; (5) one page-title size and heavier section headings; (6) two-column directory filters from 360 px; (7) the offline page left out of the saved-pages list; (8) a recapture after the changes, and the mark beside the header wordmark (optional). It judged the icon acceptable, the palette, quoted source slips, visible field edges, and the report safety notice strong. **Applied now:** (6) and (7), verified by the directory and offline browser suites. **Open:** (1) is with the maintainer's translation pass; (2) to (5) and (8) are design work not done in this run, so the visual finish is **not** accepted as `ship`, and a `fix` verdict is not whole-surface approval.
...
> **Execution status (2026-09-20): complete locally; not deployed.** `apps/web/Dockerfile` is a pinned (digest), multi-stage, non-root (uid 10001) build of the standalone output; the build receives no secret and no network path to the private API, and the runtime reads `API_INTERNAL_URL` and the internal credential from the platform's secrets only (`docs/DEPLOYMENT.md`, `railway/web.railway.json`). `apps/web/scripts/serve.mjs` is the entry point and adds a real graceful shutdown (stop accepting, finish in-flight requests, close idle connections, exit 0, at most 20 s), because the framework's own handler exited at once with code 143; two unit tests run it against a slow stand-in server. `/health/live` (no dependency) and `/health/ready` (own configuration only, no value revealed) are excluded from locale negotiation. `scripts/verify-web-container.sh` (`make web-container-verify`, with `TRIVY=1`) built the image and passed every check on 2026-09-20: non-root; OCI revision label equals the commit; no tests, public source maps, credentials, npm, corepack, or git; liveness and readiness under `--read-only`, `--cap-drop ALL`, and `no-new-privileges`; a static page served with the private API unreachable; security headers present and no HSTS over local HTTP; the credential absent from logs; clean SIGTERM exit; and no fixable high or critical vulnerability (three libpcre2 findings in the pinned base were removed by upgrading that one package). Limits: the "in-flight download" step proves the signal is handled and the exit is clean but a static file is buffered by the kernel, so draining is proven by the unit tests, not the container step; nothing was deployed.

> **Update (2026-09-20): deployed to Railway staging.** The web service was created in `shaidago-staging` and deployed from this repository's image; `https://web-staging-0edf.up.railway.app` serves it, reaching the private API over Railway's private network (the API listens on port 8080). Its build and health checks passed. The Free plan's five-service limit forced deletion of the finished `migrate` job (re-create it from `railway/migrate.railway.json` for the next migration). Details: `docs/DEPLOYMENT.md`.
...
> **Execution status (2026-09-20): partial.** The production headers (CSP, frame denial, `same-origin` referrer, nosniff, permissions policy, opener policy, and HSTS for deployed stages only), secure cookies in the deployed stages, revalidated public pages, and no-store private routes are configured and proven on the local build and inside the container (`client-security.spec.ts`, the container script). **Not done:** validating them on a hosted route and confirming that the private API is unreachable from the internet, because deploying is a maintainer action; the exact commands to run against the deployed origin are in `docs/DEPLOYMENT.md` ("Hosted verification").
...
> **Execution status (2026-09-20): blocked.** The hosted smoke matrix needs a deployed origin, which this run must not create. Everything it lists has a local, deterministic equivalent that passes (the ten journeys in four languages and two engines, offline revisit, low-data, reviewer transition and publication, public and private discovery, expired session, provider replay labels, and a deliberate dependency outage), but none was run against a hosted origin and no link was checked. Unblock: the maintainer deploys (`docs/DEPLOYMENT.md`), then runs the matrix and the link check signed out and records the date, commit, image, browsers, and viewports.

> **Update (2026-09-20): hosted checks run.** The headers, cache policy, HSTS, no-store private routes, and worker headers were confirmed on the hosted routes, the private API hostname is neither named in anything served nor reachable from outside, and a deliberate outage of the API left liveness and the offline page up and showed safe degraded pages (`docs/DEPLOYMENT.md`, `docs/evidence/FE-162-hosted-smoke.md`). Task now **complete** for what a hosted run can show; the reviewer steps still need a reviewer credential.
...
> **Execution status (2026-09-20): partial.** The root `README.md` now describes the real state (built and locally verified, not deployed), gives the verified commands (`make setup`, `make backend-verify`, `make web-verify`, `make web-verify-full`, `make verify`), architecture, six screenshots of narrow mobile, desktop project evidence, report safety, reviewer queue and detail, and Source Scout (synthetic data only, no one-time credential), trust and privacy commitments, accessibility and low-bandwidth facts, how AI was used, and the limitations, under the MIT licence. `docs/DEMO_SCRIPT.md` is a sub-four-minute path built on the six-project narrative without stating any fact about a real project. `docs/AI_BUILD_LOG.md` is current. Not done: no demo or video link exists (none is claimed), and no hosted link was checked.

> **Update (2026-09-20): hosted smoke run.** `apps/web/tests/hosted/hosted-smoke.spec.ts` ran against the deployed origin on Chromium and iPhone-13 WebKit: 23 passed and 3 skipped in the first run, and 21 passed and 3 skipped in the run after the synthetic fixture record was removed (the two Source Scout steps were rewritten to assert a stated end state, because recorded replays exist only for the synthetic fixture). Covered: the four locales with real seeded data, a citation to its source page, a refused unanswerable question, an anonymous fictional report with a one-time code and a public-safe tracking result, offline revisit with low-data mode (Chromium), and service-worker inspection. Skipped: WebKit offline and the reviewer steps (no credential). The external-link check (`scripts/check_links.py --external`) found 0 broken links of 177 checked. **Removal of the synthetic record:** at the maintainer's request `Fixture Scenario Success` was set to `visibility = 'hidden'` in the staging database (reversible with `update app.projects set visibility='public' where slug='fixture-scenario-success'`); it no longer appears in any list, and its dependent fictional rows were left intact for the audit history. A temporary SSH key was used for that one change and then removed from the account.
...
> **Execution status (2026-09-20): partial.** `make setup` and `make verify` now exist (`verify` is `backend-verify` then `web-verify-full`). On 2026-09-20, on the final tree, `make backend-verify` passed (2,096 tests, 93.9% branch coverage against 85%, OpenAPI and frontend-contract drift checks, Bandit, pip-audit), `make web-verify` passed (767 unit and component tests, coverage above the floor, bundle scan clean), and the browser suites passed (447 end-to-end passed and 11 skipped, 356 accessibility passed); the earlier frontend clean-checkout run passed as well. Not done, and each is a maintainer or hosted action: `make verify` as one command from a clean clone with an empty database, twice; a green GitHub required-checks run (nothing was pushed); the hosted revision matching `main`; a submission tag; and a triage of any new security findings. Targeted runs are not reported as full passes anywhere here. A first backend run failed seven integration tests only because the local ClamAV container was unhealthy; after restarting it the same gate passed.

> **Gate status (2026-09-20): open.** Met locally: the image is built, hardened, scanned, and behaves under a read-only filesystem with a graceful shutdown; the headers are configured and proven on the local build and in the container; the README, screenshots, demo script, and build log are current; and the backend and frontend gates pass on the final tree. Open and needing the maintainer: deploying the web service, the hosted header and smoke checks, the external-link check, a demo video and hosted URL, the clean-clone `make verify` run twice, the green hosted CI run, the submission tag, and a `main` that matches the hosted revision. A signed-out judge cannot yet open a hosted demo.

> **Update (2026-09-20): README and log.** The README now links the hosted staging demo. The AI build log records the maintainer's review and merge of pull requests #31 to #36 on every entry that said "none yet". Still open for the maintainer: a demo video; the clean-clone `make verify` run twice; a `main` that matches the hosted revision (the hosted web service was deployed from the branch `release/review-close-and-deploy`, so merge it and redeploy from `main`); the submission tag; the screen-reader smoke; and the fluent Hausa, Igbo, and Yoruba review, which the maintainer is completing.
