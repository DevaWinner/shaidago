...
## 2026-09-21 — Reorganize the Field Ledger interface

- **Task:** Modernize and reorganize every public and reviewer surface while retaining the accepted Field Ledger colours and typography; make navigation complete, location-neutral, responsive, and easy to scan.
- **User outcome delivered:** The public shell now separates brand/language utilities from task navigation, keeps project records, trust, reporting, tracking, and reviewer sign-in consistently reachable, and shows language selection at the top right. The landing page has a balanced source-record hero and search surface. Directory, reporting, tracking, handle, offline, reviewer sign-in, queue, detail, and trust pages use one page-header pattern and aligned content gutter. Text and symbolic stand-ins were replaced with semantic Lucide icons.
- **Routes/components changed:** Public and reviewer page shells under `apps/web/app/[locale]`, shared shell, landing, directory, evidence, feedback, toast, reviewer frame, new `PageHeader`, PWA/manifest icon assets, and their component, unit, end-to-end, and accessibility tests.
- **Architecture/contract impact:** No BFF, API, OpenAPI, database, worker, authentication, publication, or privacy policy changed. Browser requests remain same-origin and the backend remains the domain authority.
- **Dependency:** Added exact `lucide-react@1.47.0` for consistent accessible semantic icons. It is ISC-licensed, actively maintained, and named ESM imports are tree-shakeable; no icon font, runtime request, telemetry, or external asset host is introduced.
- **Privacy/security impact:** No new storage, cookie, analytics, tracking, provider call, or private-data surface. Reviewer and report caching rules are unchanged. Screenshots and browser fixtures use fictional records only.
- **Accessibility/localisation:** All actions retain visible text; icon-only affordances keep accessible names. Active navigation uses `aria-current`, forced-colour state has a system-colour outline, controls remain at least 44 px, navigation wraps at 200% text, and every page was exercised in all four complete locales.
- **Verification:** `pnpm --dir apps/web typecheck`, lint, formatting, contract generation check, message check, and webpack production build passed. Coverage passed 769 tests at 91.46% statements and 84.95% branches. The complete accessibility suite passed 356/356 across Chromium and mobile WebKit. The canary client-boundary build verified 76 JavaScript chunks. `git diff --check` passed. The standard Turbopack build could not bind its internal local port in the managed sandbox; the equivalent Next.js webpack production build passed twice. Impeccable's final detector returned no findings.
- **Visual evidence:** Final full-page captures of English desktop and Hausa mobile renders were inspected; layout, navigation, localized copy, responsive stacking, CTA hierarchy, iconography, and footer were visually sound. Captures contain synthetic test records and were kept outside the repository.
- **Known limitations:** The prior independent finish review returned `fix`; this change addresses its recorded design findings but has not received a fresh independent disposition. The manual screen-reader smoke and independent second-language review remain open.
- **Commit/PR:** pending at the time of this entry.
- **AI assistance used:** Applied the Impeccable UI workflow, reorganized information architecture and components, migrated iconography, fixed forced-colour and 200% zoom regressions, updated localized test targeting, and verified rendered output.
- **Prompt summary:** Retain the Field Ledger visual identity while making every page modern, aligned, location-neutral, navigable, icon-consistent, and complete across the four languages.
- **Human review:** Pending pull-request review by the maintainer.
