...
- **Status:** evidence for FE-140 to FE-145. Automated proof is listed with the command that produces it. Items that still need an independent person (screen-reader smoke and second-language review) are written as scripts below and are **not** claimed as done.
...
- **Translation status (visible in `apps/web/messages/status.json`):** English, Hausa, Igbo, and Yoruba each contain all 1,125 messages with matching keys and ICU variables and no null fallback values. Every domain is critical and was marked maintainer-reviewed on 2026-09-20. Hausa, Igbo, and Yoruba have no independent second-language sign-off; the status file states that limitation explicitly.
...
1. Screen-reader smoke (section 1) and independent second-language review (section 2) still need people.
