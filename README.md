...
- **Frontend.** Only staging is deployed (Railway Free plan, generated domain); the earlier independent finish review returned `fix`, and this reorganization addresses its recorded navigation, hierarchy, alignment, icon, and first-viewport findings but has not received a new independent disposition; the screen-reader smoke has not been run; the CSP keeps `unsafe-inline` for scripts and styles (a recorded trade-off); and Playwright cannot test offline navigation with a service worker on WebKit, so that proof is Chromium-only.
