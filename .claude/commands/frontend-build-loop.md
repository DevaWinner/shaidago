...
2. Determine the selected task's circle and derive a concise kebab-case branch name from that circle's product goal: `frontend/<circle-goal>`. Examples: `frontend/contract-inventory`, `frontend/web-foundation`, and `frontend/bff-boundary`. Do not use a roadmap label, circle number, or task ID in a branch name.
3. Each circle has its own branch. If the current branch is not that circle's `frontend/<circle-goal>` branch, create and switch to it from the current committed baseline. If it already exists, switch only after confirming it contains the required predecessor work; otherwise stop and record the branch conflict. Do not merge, rebase, force-push, or alter the backend branch that supplied the frontend baseline. Never commit loop work directly to `main`.
4. Never stage `CLAUDE.md`. Do not stage an untracked item under `.claude/`; a command file already tracked by Git may be updated intentionally.
...
5. **Commit.** Stage intentional files by explicit path; never use `git add -A` or `git add .`. Commit with a Conventional Commit subject matching repository history (`feat: …`, `fix: …`, `build: …`, `test: …`, `docs: …`, `ci: …`; scope optional) and describing the delivered outcome. Do not put a roadmap label, circle number, or `FE-` task ID in the branch name or commit subject. **Do not add a `Co-Authored-By` trailer or any other attribution line to the commit message.** One commit per task, or per tightly coupled pair only where build-order section 1.3 permits it. Never amend, rebase, force-push, or rewrite history.
6. **Circle transition.** After a circle's final task, check every exit-gate criterion and record the result in a `> **Gate status (YYYY-MM-DD): …**` note under that circle's exit-gate heading. Fix any criterion that is within scope before moving on. If the only missing criterion is a hard stop, mark the gate open and name the missing evidence. Before starting any eligible task in a different circle, create or switch to that circle's separate `frontend/<circle-goal>` branch from the just-recorded baseline.
